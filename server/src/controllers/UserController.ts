...
class UserController {
    signIn(session: any, username: string) {
        ...
        return {message: `Successfully logged in! Welcome ${username}.`, userId: account.userID, username:account.username};     
    }
    ...
    getLoginStatus(session: any) {
        ...
        let account = UserRepository.findOneByID(sessionId);
        let response : any = {message: `You are ${sessionId ? '' : 'not'} signed in ${sessionId ? ` as ${account.username}` : '.'}`, signedIn: (sessionId ? true : false)};
        if (sessionId) {
